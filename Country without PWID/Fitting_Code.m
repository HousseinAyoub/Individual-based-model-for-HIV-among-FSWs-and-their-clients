...
Cond_sp=0.015; %2.9% of acts with spouses are coovered by condom use
...
for t=t0+YearBURNIN:DT:tf+1-DT
    %%%Coverage MC
    if t<2020
      CoverageMC(round((t-t0)*Unit+1))=0.999; % This is the baseline coverage for MC for South-Sudan
    else
      CoverageMC(round((t-t0)*Unit+1))=0.999;  %%This needs to be updated according to the intervention scenario
    end
    
    %%%Coverage Cond
    if t<2020
      CoverageCond(round((t-t0)*Unit+1))=0.523; % This is the baseline coverage for Condome use for South-Sudan
    else
      CoverageCond(round((t-t0)*Unit+1))=0.523;  %%This needs to be updated according to the intervention scenario (fox example 0.5 or 0.81)
    end
    
    %%%Coverage ART
    if t<2020
      CovARTFSW(round((t-t0)*Unit+1))=0.57; % This is the baseline coverage for ART in FSWs for South-Sudan
    else
      CovARTFSW(round((t-t0)*Unit+1))=0.57;  %%This needs to be updated according to the intervention scenario (fox example 0.25 or 0.5 or 0.81)
    end
    
    if t<2020
      CovARTClient(round((t-t0)*Unit+1))=0.57; % This is the baseline coverage for ART for South-Sudan for general population
    else
      CovARTClient(round((t-t0)*Unit+1))=0.57;  %%This needs to be updated according to the intervention scenario (fox example 0.25 or 0.5 or 0.81)
    end
