...
%%%%%%%%%%%%%%%%%%%%%%%%HIV among PWID%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for cnt= 1:fsw
      if (IDU_fsw(cnt)==1) && (state_fsw(cnt)==0) 
          if rand<HIVPWID*(1-(CovPrEPFSW(t)*ePrEP))*(1-(CovARTFSW(t)*eART))
             state_fsw(cnt)=1;
...
%%%Added as Test on 07/13/20 to check impact of effectivness on the ART
%%%intervention
eART=eARTT(t);
                       ...
                       for cnt2 = 1:fsw % for FSWs
                           if rand<CovPrEPF %if the FSW is on PrEP (ART+ & Cond+ &PrEP+)
                              ...
                              incidence_fsw = incidence_fsw + 1; % count incidence of fsw
                                    if IDU_fsw(cnt2)==0
                                       incidence_fsw_notIDU=incidence_fsw_notIDU+1;
                                    end
                              end
...
prevalence_fsw_IDU = 0; % reset counter for prevalence in FSWs who inject drugs
prevalence_fsw_notIDU = 0; % reset counter for prevalence in FSWs who do NOT inject drugs
...
susceptible_fsw_IDU = 0;   % reset counter for susceptible FSWs who inject drugs
susceptible_fsw_notIDU = 0; % reset counter for susceptible FSWs who do NOT inject drugs

    for cnt = 1:fsw
        if IDU_fsw(cnt) == 0
           if state_fsw(cnt) ~= 0
              prevalence_fsw_notIDU = prevalence_fsw_notIDU + 1;
           else
              susceptible_fsw_notIDU = susceptible_fsw_notIDU + 1;
           end
        else
           if state_fsw(cnt) ~= 0
              prevalence_fsw_IDU = prevalence_fsw_IDU + 1;
           else
              susceptible_fsw_IDU = susceptible_fsw_IDU + 1;
           end
        end
     end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       Number_I_FSW(t,1)=prevalence_fsw_IDU+prevalence_fsw_notIDU;     %Number of infected FSWs (injecting and non-injecting)
       ...
       Number_I_FSWIDU(t,1)=prevalence_fsw_IDU;                        %Number of infected FSWs who inject drugs
       Number_I_FSWnotIDU(t,1)=prevalence_fsw_notIDU;                  %Number of infected FSWs who do Not inject drugs
       ...
       Inc_IDU(t,1)=incidence_fsw_IDU;                                 %Total number of new infections among FSWs due to injecting drug use
       Inc_notIDU(t,1)=incidence_fsw_notIDU;                           %Total number of new infections among FSWs who do not inject drugs
       Incidence(t,1)=incidence_fsw + incidence_fsw_IDU;               %Total number of new infections among FSWs
       
       Susceptible(t,1)=susceptible_fsw_notIDU+susceptible_fsw_IDU;    %Number of susceptible FSWs (injecting and non-injecting)
       ...
       Susceptible_IDU(t,1)=susceptible_fsw_IDU;                       %Number of susceptible FSWs among FSWs who inject drugs
       Susceptible_notIDU(t,1)=susceptible_fsw_notIDU;                 %Number of susceptible FSWs among FSWs who do NOT inject drugs
       
       N_fsw_PWID(t,1)=fsw_PWID;                                       %Number of FSWs who are PWID
       N_fsw_notPWID(t,1)=fsw_notPWID;                                 %Number of FSWs who are not PWID
end

for t=1:YearEND
    
    NPWID(t)=N_fsw_PWID((t*Unit),1);                                        %Number of FSWs who are PWID 
    NnotPWID(t)=N_fsw_notPWID((t*Unit),1);                                  %Number of FSWs who do not inject drugs    
    
    InfectedFSW(t)=Number_I_FSW((t*Unit),1);                                %Number of infected FSWs (injecting and non-injecting) at 12 months, 24 months...
    InfectedFSWIDU(t)=Number_I_FSWIDU((t*Unit),1);                          %Number of infected FSWs who inject drugs at 12 months, 24 months...
    InfectedFSWnotIDU(t)=Number_I_FSWnotIDU((t*Unit),1);                    %Number of infected FSWs who do not inject drugs at 12 months, 24 months...
    ...
    Susc_IDU(t)=Susceptible_IDU((t*Unit),1);                                %Number of susceptible FSWs who inject drugs at 12 months, 24 months..
    Susc_notIDU(t)=Susceptible_notIDU((t*Unit),1);                          %Number of susceptible FSWs who do not inject drugs at 12 months, 24 months..
    ...
    Incidence1YearIDU(t)=sum(Inc_IDU(1+(t-1)*Unit:t*Unit,1));               %Total number of new infections per year due to IDU for xxx years among FSWs who inject drugs, that is incidence from 0:12 months, 12:24 months ....
    Incidence1YearnotIDU(t)=sum(Inc_notIDU(1+(t-1)*Unit:t*Unit,1));         %Total number of new infections per year due to sexual transmission for xxx years among FSWs who do not inject drugs, that is incidence from 0:12 months, 12:24 months ....
    ...
    PrevalenceYear1IDU(t)=100*InfectedFSWIDU(t)/NPWID(t);                   %HIV prevalence per year among FSWs who inject drugs at 12 months, 24 months...
    PrevalenceYear1notIDU(t)=100*InfectedFSWnotIDU(t)/NnotPWID(t);          %HIV prevalence per year among FSWs who do not inject drugs at 12 months, 24 months...
    ...
    IncidenceR1YearIDU(t)=100*Incidence1YearIDU(t)/Susc_IDU(t);             %Incidence rate among FSWs who inject drugs
    IncidenceR1YearnotIDU(t)=100*Incidence1YearnotIDU(t)/Susc_notIDU(t);    %Incidence rate among FSWs who do not inject drugs

   
    eps=1.0E-10;
    IncContYearS(t)=100*(Incidence1YearS(t)/(Incidence1Year(t)+eps));       %Contribution of sexual transmission in FSWs to total incidence in FSWs
    IncContYearIDU(t)=100*(Incidence1YearIDU(t)/(Incidence1Year(t)+eps));   %Contribution of injecting dug use in FSWs to total incidence in FSWs

    Ratio(t)=IncContYearS(t)/IncContYearIDU(t);                             %Ratio of contribution of sexual transmission and injecting drug use in FSWs
...
IncidenceContSexual=zeros(YearEND,1);
IncidenceContInjecting=zeros(YearEND,1);

for t=50:YearEND
    IncidenceContSexual(t)=100*sum(Incidence1YearS(t-49:t))/sum(Incidence1Year(t-49:t));        %Contribution of sexual transmission in FSWs to total incidence in FSWs-moving average over 50 years
    IncidenceContInjecting(t)=100*sum(Incidence1YearIDU(t-49:t))/sum(Incidence1Year(t-49:t));   %Contribution of injecting drug use in FSWs to total incidence in FSWs-moving average over 50 years
end
...
Susc_IDU_Avg=zeros(YearEND,1);
Susc_notIDU_Avg=zeros(YearEND,1);
...
IncidenceR1YearIDU_Avg=zeros(YearEND,1);
IncidenceR1YearnotIDU_Avg=zeros(YearEND,1);
    
InfectedFSW_Avg=zeros(YearEND,1);                           
InfectedFSWIDU_Avg=zeros(YearEND,1);                      
InfectedFSWnotIDU_Avg=zeros(YearEND,1);                 
...
PrevalenceYear1IDU_Avg=zeros(YearEND,1); 
PrevalenceYear1notIDU_Avg=zeros(YearEND,1);
...
for t=50:YearEND
    ...
    Susc_IDU_Avg(t)=sum(Susc_IDU(t-49:t))/50;                               %average number of susceptible FSWs among FSWs who inject drugs
    Susc_notIDU_Avg(t)=sum(Susc_notIDU(t-49:t))/50;                         %average number of susceptible FSWs among FSWs who do not inject drugs
    ...
    Incidence1YearIDU_Avg(t)=sum(Incidence1YearIDU(t-49:t))/50;             %average number of new infections per year due to IDU among FSWs who inject drugs
    Incidence1YearnotIDU_Avg(t)=sum(Incidence1YearnotIDU(t-49:t))/50;       %average number of new infections per year among FSWs who do not inject drugs
    ...
    IncidenceR1YearIDU_Avg(t)=100*Incidence1YearIDU_Avg(t)/Susc_IDU_Avg(t);     %average incidence rate among FSWs who inject drugs
    IncidenceR1YearnotIDU_Avg(t)=100*Incidence1YearnotIDU_Avg(t)/Susc_notIDU_Avg(t); %average incidence rate among FSWs who do not inject drugs
    
    InfectedFSW_Avg(t)=sum(InfectedFSW(t-49:t))/50;                             %average number of infected FSWs (injecting and non-injecting) per year
    InfectedFSWIDU_Avg(t)=sum(InfectedFSWIDU(t-49:t))/50;                       %average number of infected FSWs per year among FSWs who inject drugs
    InfectedFSWnotIDU_Avg(t)=sum(InfectedFSWnotIDU(t-49:t))/50;                 %average number of infected FSWs per year among FSWs who do not inject drugs
    ...
    PrevalenceYear1IDU_Avg(t)=100*InfectedFSWIDU_Avg(t)/NPWID(t);               %average HIV prevalence among FSWs who inject drugs 
    PrevalenceYear1notIDU_Avg(t)=100*InfectedFSWnotIDU_Avg(t)/NnotPWID(t);      %average HIV prevalenceamong FSWs who do not inject drugs 
    ...
    IncR_sp_Avg(t)=100*Inc_sp_Avg(t)/Susc_sp_Avg(t);                            %average i
end